import { FOLLOW_UP_STATUS } from '../constants/leadStatuses'
...
export function parseLeadsFromWorksheet(rows) {
  ...
  const leads = dataRows
    .map((row, index) => {
      ...
      const status = getValue('status') || FOLLOW_UP_STATUS
...
export const EXPECTED_EXCEL_COLUMNS = 'Name, Email, Phone, Company, Role, Status (optional)'
